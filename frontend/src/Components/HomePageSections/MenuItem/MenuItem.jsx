...
const MenuItem = ({ imgUrl, title, price, newPrice, minWidth, minHeight, hover, children, _id, setCartItems }) => {
  function isHover(){
    if(hover){
      return 'hover';
    } else {
  ...
  return (
    <div className={`menu-card-item ${isHover()}`}>
      ...
      <ButtonAnimation setCartItems={setCartItems} id={_id} style={{fontSize: '1.2rem'}} title={'Add to Cart'} />
    </div>
  )
}
